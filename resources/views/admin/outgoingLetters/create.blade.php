...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.outgoingLetter.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.outgoing-letters.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('delivered_date') ? 'is-invalid' : '' }}" type="text" name="delivered_date" id="delivered_date" value="{{ old('delivered_date') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('outgoing_type') ? 'is-invalid' : '' }}" name="outgoing_type" id="outgoing_type" required>
                    ...
                    @foreach(App\Models\OutgoingLetter::OUTGOING_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('outgoing_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('incoming_letter') ? 'is-invalid' : '' }}" name="incoming_letter_id" id="incoming_letter_id">
                    @foreach($incoming_letters as $id => $entry)
                        <option value="{{ $id }}" {{ old('incoming_letter_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
