    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.course-students.update", [$courseStudent->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('beneficiary') ? 'is-invalid' : '' }}" name="beneficiary_id" id="beneficiary_id">
                    @foreach($beneficiaries as $id => $entry)
                        <option value="{{ $id }}" {{ (old('beneficiary_id') ? old('beneficiary_id') : $courseStudent->beneficiary->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('beneficiary_family') ? 'is-invalid' : '' }}" name="beneficiary_family_id" id="beneficiary_family_id">
                    @foreach($beneficiary_families as $id => $entry)
                        <option value="{{ $id }}" {{ (old('beneficiary_family_id') ? old('beneficiary_family_id') : $courseStudent->beneficiary_family->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $courseStudent->note) }}</textarea>
                @if($errors->has('note'))
                    <div class="invalid-feedback">
                        {{ $errors->first('note') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('prev_courses') ? 'is-invalid' : '' }}" name="prev_courses" id="prev_courses">{{ old('prev_courses', $courseStudent->prev_courses) }}</textarea>
