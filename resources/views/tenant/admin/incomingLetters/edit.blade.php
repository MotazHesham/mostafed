...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.incomingLetter.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.incoming-letters.update", [$incomingLetter->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="letter_number">{{ trans('cruds.incomingLetter.fields.letter_number') }}</label>
                <input class="form-control {{ $errors->has('letter_number') ? 'is-invalid' : '' }}" type="text" name="letter_number" id="letter_number" value="{{ old('letter_number', $incomingLetter->letter_number) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('external_number') ? 'is-invalid' : '' }}" type="text" name="external_number" id="external_number" value="{{ old('external_number', $incomingLetter->external_number) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('letter_date') ? 'is-invalid' : '' }}" type="text" name="letter_date" id="letter_date" value="{{ old('letter_date', $incomingLetter->letter_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('received_date') ? 'is-invalid' : '' }}" type="text" name="received_date" id="received_date" value="{{ old('received_date', $incomingLetter->received_date) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('recevier') ? 'is-invalid' : '' }}" name="recevier_id" id="recevier_id" required>
                    @foreach($receviers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('recevier_id') ? old('recevier_id') : $incomingLetter->recevier->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('subject') ? 'is-invalid' : '' }}" type="text" name="subject" id="subject" value="{{ old('subject', $incomingLetter->subject) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('letter') ? 'is-invalid' : '' }}" name="letter" id="letter">{!! old('letter', $incomingLetter->letter) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('priority') ? 'is-invalid' : '' }}" name="priority" id="priority" required>
                    ...
                    @foreach(App\Models\IncomingLetter::PRIORITY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('priority', $incomingLetter->priority) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('incoming_type') ? 'is-invalid' : '' }}" name="incoming_type" id="incoming_type" required>
                    ...
                    @foreach(App\Models\IncomingLetter::INCOMING_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('incoming_type', $incomingLetter->incoming_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('letter_organization') ? 'is-invalid' : '' }}" name="letter_organization_id" id="letter_organization_id">
                    @foreach($letter_organizations as $id => $entry)
                        <option value="{{ $id }}" {{ (old('letter_organization_id') ? old('letter_organization_id') : $incomingLetter->letter_organization->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $incomingLetter->note) }}</textarea>
                @if($errors->has('note'))
                    <div class="invalid-feedback">
                        {{ $errors->first('note') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('outgoing_letter') ? 'is-invalid' : '' }}" name="outgoing_letter_id" id="outgoing_letter_id">
                    @foreach($outgoing_letters as $id => $entry)
                        <option value="{{ $id }}" {{ (old('outgoing_letter_id') ? old('outgoing_letter_id') : $incomingLetter->outgoing_letter->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
