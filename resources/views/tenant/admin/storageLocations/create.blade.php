...
@section('content')
    @php
        $breadcrumbs = [
            ...
            ['title' => trans('global.create') . ' ' . trans('cruds.storageLocation.title_singular'), 'url' => '#'],
        ];
    @endphp
    ...
    <div class="card">
        <div class="card-header p-3">
            <h6 class="card-title">
                {{ trans('global.create') }} {{ trans('cruds.storageLocation.title_singular') }}
            </h6>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('admin.storage-locations.store') }}" enctype="multipart/form-data">
                @csrf
