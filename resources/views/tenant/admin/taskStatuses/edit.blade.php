...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.task-statuses.update", [$taskStatus->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.taskStatus.fields.badge_class') }}</label>
                <select class="form-control {{ $errors->has('badge_class') ? 'is-invalid' : '' }}" name="badge_class" id="badge_class" required>
                    ...
                    @foreach(App\Models\TaskStatus::BADGE_CLASS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('badge_class', $taskStatus->badge_class) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.taskStatus.fields.badge_class_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="ordering">{{ trans('cruds.taskStatus.fields.ordering') }}</label>
                <input class="form-control {{ $errors->has('ordering') ? 'is-invalid' : '' }}" type="number" name="ordering" id="ordering" value="{{ old('ordering', $taskStatus->ordering) }}" step="1">
                @if($errors->has('ordering'))
                    <div class="invalid-feedback">
                        {{ $errors->first('ordering') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.taskStatus.fields.ordering_helper') }}</span>
            </div>
            <div class="form-group">
