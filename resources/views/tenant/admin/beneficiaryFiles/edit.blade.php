...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.beneficiaryFile.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.beneficiary-files.update", [$beneficiaryFile->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('beneficiary') ? 'is-invalid' : '' }}" name="beneficiary_id" id="beneficiary_id">
                    @foreach($beneficiaries as $id => $entry)
                        <option value="{{ $id }}" {{ (old('beneficiary_id') ? old('beneficiary_id') : $beneficiaryFile->beneficiary->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $beneficiaryFile->name) }}">
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('required_document') ? 'is-invalid' : '' }}" name="required_document_id" id="required_document_id">
                    @foreach($required_documents as $id => $entry)
                        <option value="{{ $id }}" {{ (old('required_document_id') ? old('required_document_id') : $beneficiaryFile->required_document->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
