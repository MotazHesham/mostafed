...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.beneficiaryOrder.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.beneficiary-orders.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\BeneficiaryOrder::ACCEPT_STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('accept_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="accept_status_{{ $key }}" name="accept_status" value="{{ $key }}" {{ old('accept_status', '') === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('refused_reason') ? 'is-invalid' : '' }}" name="refused_reason" id="refused_reason">{{ old('refused_reason') }}</textarea>
